...
df_top_operator = tbl_users_state.groupby(["raw_sim_operator_name", "region"])['device_id'].count()
# convert the Series to a DataFrame
df_top_operator = df_top_operator.reset_index()
# rename columns for clarity
df_top_operator.columns = ['raw_sim_operator_name', "region", 'device_id_count']
# obtain the total number of users per region
totales = df_top_operator.groupby("region")["device_id_count"].aggregate(usuarios_region = "sum").reset_index()
# if not in 10 top operators, then assign 'other'
df_top_operator.loc[~df_top_operator["raw_sim_operator_name"].isin(top_operators), "raw_sim_operator_name"] = "other"
...
# Wrangle P2
# Get market share
df_top_operator["market_share"] = df_top_operator["device_id_count"]/ df_top_operator["usuarios_region"]
