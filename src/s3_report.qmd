...
En el siguiente mapa se puede observar la presencia nacional de Bait en cada una de las tiendas (puntos de venta). El tamaño de la burbuja nos indica el número de clientes Bait por tienda. Un tamaño de burbuja mayor está mapeado a un color rojo intenso y un tamaño menor a un color rojo claro. Para decidir si un cliente Bait es de una tienda en específico a cada conexión registrada en la base de datos se le asigna la tienda más cercana. Posteriormente, a cada usuario registrado, se le asigna la tienda donde se hayan registrado el mayor número de conexiones.
...
```{python}
def plot_store_analysis(store_potential_users, store_real_users):
    # User analysis for a store
...
plot_store_analysis(potential_users, real_users)
```
...
cp_top = str(gdf_walmart[gdf_walmart["name"]==users_per_store.head(n=1)["Nombre"].iloc[0]].postal_code.iloc[0])
...
```{python}
plot_store_analysis(potential_users_top, real_users_top)
```
...
# Filter the day with less traffic
less_traffic_bottom = min(potential_users_bottom["count"])
...
# Filter the day with less users
less_users_bottom = min(potential_users_bottom["count"])
...
```{python}
# User analysis for the less crowded store
plot_store_analysis(potential_users_bottom, real_users_bottom)
```